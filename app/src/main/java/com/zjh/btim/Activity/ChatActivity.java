...
public class ChatActivity extends AppCompatActivity implements View.OnClickListener {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case BLUE_TOOTH_READ:
                    ...
                    Log.i("zjhChat", deviceName + ":" + readMessage);
                ...
                case BLUE_TOOTH_WRAITE:
                    ...
                    Log.i("zjhChat", "I" + ":" + writeMessage);
                    list.add(new ChatInfo(ChatInfo.TAG_RIGHT, "I", writeMessage));
                    recyclerChatAdapter.notifyDataSetChanged();
                    recyclerView.smoothScrollToPosition(list.size());
                    dbManager.add(new ChatRecord(deviceMac, ChatInfo.TAG_RIGHT, "I", writeMessage));
                    break;
                case BLUE_TOOTH_READ_FILE_NOW:
                    Log.i("zjhTransfert", msg.obj + "");
                    Snackbar.make(et_write, (String) msg.obj, Snackbar.LENGTH_LONG).show();
                    break;
                case BLUE_TOOTH_WRAITE_FILE_NOW:
                    Log.i("zjhTransfert", msg.obj + "");
                    if (msg.obj.toString().equals("Échec de la livraison d'un fichier")) {
                        dialog.dismiss();
                        Snackbar.make(et_write, (String) msg.obj, Snackbar.LENGTH_LONG).show();
                    } else {
                ...
                case BLUE_TOOTH_READ_FILE:
                    Log.i("zjhTransfert", "Réception du document terminée(" + msg.obj + ")");
                ...
                case BLUE_TOOTH_WRAITE_FILE:
                    Log.i("zjhTransfert", "Livraison du fichier terminée");
                    dialog.dismiss();
                    list.add(new ChatInfo(ChatInfo.TAG_FILE_RIGHT, "I", msg.obj + ""));
                    recyclerChatAdapter.notifyDataSetChanged();
                    recyclerView.smoothScrollToPosition(list.size());
                    dbManager.add(new ChatRecord(deviceMac, ChatInfo.TAG_FILE_RIGHT, "I", msg.obj + ""));
                    break;
                case UPDATE_DATA:
                    recyclerChatAdapter.notifyDataSetChanged();
                    recyclerView.smoothScrollToPosition(list.size());
    ...
    private void send() {
        String write = et_write.getText().toString().trim();
        if (TextUtils.isEmpty(write)) {
            Snackbar.make(et_write, "Le contenu de l'envoi ne peut pas être vide", Snackbar.LENGTH_LONG).show();
            return;
        }
        ...
        XPermissionUtil.requestPermissions(this, 1,
                ...
                new XPermissionUtil.OnPermissionListener() {
                    ...
                    public void onPermissionGranted() {
                        //Permissions obtenues avec succès
                        bluetoothChatService.sendFile(file);
                    }

                    @Override
                    public void onPermissionDenied() {
                        //Échec de l'acquisition de la permission
                        Snackbar.make(et_write, "Veuillez accéder manuellement à l'écran des paramètres pour accorder les autorisations nécessaires.", Snackbar.LENGTH_LONG).show();
                    }
                });
    }
        ...
        switch (requestCode) {
            case FILE_SELECT_CODE:
                if (resultCode == RESULT_OK) {
                    ...
                    Log.i("zjh-Sélectionner un fichier", "File Path: " + path);
                    if (path != null) {
                        ...
                        ad.setMessage("Êtes-vous sûr de vouloir envoyer" + path + "Est-ce le cas ?");
    ...
    private void exit() {
        ...
        ad.setNeutralButton("Minimiser", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
        ...
        ad.setMessage("Êtes-vous sûr de vouloir vous déconnecter ?");
        ad.setTitle("Conseils");
        ad.setCancelable(false);
        ad.show();
    }
